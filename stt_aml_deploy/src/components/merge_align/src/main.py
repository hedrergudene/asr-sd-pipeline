...
def run(mini_batch):
    ...
    for elem in mini_batch: # mini_batch on ASR files
        ...
        cm.decrypt(input_path, './decrypted_files', f"{fn}_nfa.json.pgp", pk_secret, pk_pass_secret)
        ...
        if len(asr_dct['segments'])==0:
            ...
            cm.encrypt('./decrypted_files', output_sm_path, f"{fn}.json", pubk_secret, True)
        ...
        cm.decrypt(input_path, './decrypted_files', f"{fn}_diar.json.pgp", pk_secret, pk_pass_secret)
