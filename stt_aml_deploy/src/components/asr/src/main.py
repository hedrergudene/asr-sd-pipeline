...
def run(mini_batch):
    ...
    for elem in mini_batch:
        ...
        cm.decrypt(input_path, './decrypted_files', f"{fn}_prep.json.pgp", pk_secret, pk_pass_secret)
        ...
        if len(metadata_dct['vad_timestamps'])==0:
            ...
            cm.encrypt('./decrypted_files', output_asr_path, f"{fn}_asr.json", pubk_secret, True)
            continue
